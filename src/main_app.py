...
class MainApplication(tk.Tk):
    ...
    def _create_top_panel(self):
        ...
        tk.Label(top_frame, text="数据文件:", bg="#f0f0f0", font=DEFAULT_FONT).pack(side=tk.LEFT, padx=5)

        self.file_path_var = tk.StringVar(value="未选择文件")
        self.file_label = tk.Label(top_frame, textvariable=self.file_path_var,
                                   bg="white", relief=tk.SUNKEN, width=60, font=DEFAULT_FONT)
        self.file_label.pack(side=tk.LEFT, padx=5)

        self.load_btn = tk.Button(top_frame, text="打开数据文件", command=self.load_file, font=DEFAULT_FONT)
        self.load_btn.pack(side=tk.LEFT, padx=5)

        self.show_data_btn = tk.Button(top_frame, text="显示已加载的数据", command=self.show_data,
                                       state=tk.DISABLED, font=DEFAULT_FONT)
        self.show_data_btn.pack(side=tk.LEFT, padx=5)

        self.data_info_var = tk.StringVar(value="")
        tk.Label(top_frame, textvariable=self.data_info_var, bg="#f0f0f0", font=DEFAULT_FONT).pack(side=tk.RIGHT)
    ...
    def _create_left_panel(self):
        ...
        tk.Label(left_frame, text="参数设置", font=('SimHei', 12, 'bold'),
                 bg="#f5f5f5").pack(pady=(0, 10))

        model_frame = tk.LabelFrame(left_frame, text="模型选择", padx=5, pady=5, font=DEFAULT_FONT)
        model_frame.pack(fill="x", pady=5)

        self.model_var = tk.StringVar(value="linear_regression")
        tk.Radiobutton(model_frame, text="多元线性回归", variable=self.model_var,
                       value="linear_regression", command=self._update_params_panel,
                       font=DEFAULT_FONT).pack(anchor=tk.W)
        tk.Radiobutton(model_frame, text="决策树回归", variable=self.model_var,
                       value="decision_tree", command=self._update_params_panel,
                       font=DEFAULT_FONT).pack(anchor=tk.W)

        self.params_frame = tk.LabelFrame(left_frame, text="模型参数", padx=5, pady=5, font=DEFAULT_FONT)
        self.params_frame.pack(fill="x", pady=5)
        self._update_params_panel()

        split_frame = tk.LabelFrame(left_frame, text="数据划分", padx=5, pady=5, font=DEFAULT_FONT)
        split_frame.pack(fill="x", pady=5)

        tk.Label(split_frame, text="训练集比例:", font=DEFAULT_FONT).grid(row=0, column=0, sticky="w")
        ...
        self.split_label = tk.Label(split_frame, text="80%", font=DEFAULT_FONT)
        ...
        std_frame = tk.LabelFrame(left_frame, text="数据预处理", padx=5, pady=5, font=DEFAULT_FONT)
        std_frame.pack(fill="x", pady=5)
        self.standardize_var = tk.BooleanVar(value=True)
        tk.Checkbutton(std_frame, text="标准化特征", variable=self.standardize_var,
                       font=DEFAULT_FONT).pack(anchor=tk.W)
    ...
    def _update_params_panel(self):
        ...
        if model_type == "linear_regression":
            tk.Label(self.params_frame, text="学习率:", font=DEFAULT_FONT).grid(row=0, column=0, sticky="w")
            self.lr_var = tk.DoubleVar(value=0.01)
            tk.Entry(self.params_frame, textvariable=self.lr_var, width=10, font=DEFAULT_FONT).grid(row=0, column=1)

            tk.Label(self.params_frame, text="迭代次数:", font=DEFAULT_FONT).grid(row=1, column=0, sticky="w")
            self.max_iter_var = tk.IntVar(value=1000)
            tk.Entry(self.params_frame, textvariable=self.max_iter_var, width=10, font=DEFAULT_FONT).grid(row=1, column=1)

            tk.Label(self.params_frame, text="正则化系数:", font=DEFAULT_FONT).grid(row=2, column=0, sticky="w")
            self.alpha_var = tk.DoubleVar(value=0.0)
            tk.Entry(self.params_frame, textvariable=self.alpha_var, width=10, font=DEFAULT_FONT).grid(row=2, column=1)
        else:
            tk.Label(self.params_frame, text="最大深度:", font=DEFAULT_FONT).grid(row=0, column=0, sticky="w")
            self.max_depth_var = tk.IntVar(value=5)
            tk.Entry(self.params_frame, textvariable=self.max_depth_var, width=10, font=DEFAULT_FONT).grid(row=0, column=1)

            tk.Label(self.params_frame, text="最小样本数:", font=DEFAULT_FONT).grid(row=1, column=0, sticky="w")
            self.min_samples_var = tk.IntVar(value=2)
            tk.Entry(self.params_frame, textvariable=self.min_samples_var, width=10, font=DEFAULT_FONT).grid(row=1, column=1)
    ...
    def _create_right_panel(self):
        ...
        metrics_frame = tk.LabelFrame(right_frame, text="评估指标", padx=5, pady=5, font=DEFAULT_FONT)
        metrics_frame.grid(row=1, column=0, sticky="ew")

        self.metrics_text = tk.Text(metrics_frame, height=6, width=60, font=DEFAULT_FONT)
    ...
    def _create_bottom_panel(self):
        ...
        self.status_bar = tk.Label(bottom_frame, textvariable=self.status_var,
                                   bg="#f0f0f0", relief=tk.SUNKEN, anchor=tk.W, font=DEFAULT_FONT)
    ...
    def load_file(self):
        ...
        try:
            ...
            self.show_data_btn.config(state=tk.NORMAL)
    ...
    def show_data(self):
        ...
        data_window.title("已加载的数据")
        data_window.geometry("800x600")

        frame = tk.Frame(data_window)
        frame.pack(fill="both", expand=True, padx=10, pady=10)

        text = tk.Text(frame, font=DEFAULT_FONT)
        text.pack(fill="both", expand=True)

        text.insert(tk.END, "=" * 80 + "\n")
        text.insert(tk.END, "已加载的数据预览\n")
        text.insert(tk.END, "=" * 80 + "\n\n")

        text.insert(tk.END, f"样本数: {len(self.X)}\n")
        text.insert(tk.END, f"特征数: {len(self.feature_names)}\n")
        text.insert(tk.END, f"特征名称: {', '.join(self.feature_names)}\n")
        text.insert(tk.END, f"目标变量: 抗压强度\n")
        text.insert(tk.END, "\n" + "=" * 80 + "\n")
        text.insert(tk.END, "数据预览（前20行）\n")
        text.insert(tk.END, "=" * 80 + "\n")

        header = "\t".join(self.feature_names) + "\t抗压强度\n"
        text.insert(tk.END, header)
        text.insert(tk.END, "-" * 80 + "\n")

        for i in range(min(20, len(self.X))):
            row_data = "\t".join(str(val) for val in self.X[i]) + "\t" + str(self.y[i]) + "\n"
            text.insert(tk.END, row_data)
    ...
    def clear_results(self):
        ...
        self.show_data_btn.config(state=tk.DISABLED)
